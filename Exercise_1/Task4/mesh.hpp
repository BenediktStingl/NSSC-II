...
public:
    ...
    void GhostSwap()
    {
        if(this->vert_src >= 0) // top
        {
            ...
            MPI_Recv(&topGhost.front(), topGhost.size(), MPI_DOUBLE, this->vert_src, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  //receives ghost
            for(long i = this->i; i < this->I; i++)
            {
        ...
        if(this->vert_dest >= 0) // bottom
        {
            ...
            MPI_Recv(&bottomGhost.front(), bottomGhost.size(), MPI_DOUBLE, this->vert_dest, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);  //receives ghost
            for(long i = this->i; i < this->I; i++)
            {
        ...
        if(this->DIM == "2D")
        {
            if(this->hor_src >= 0) // left
            {
                ...
                MPI_Recv(&leftGhost.front(), leftGhost.size(), MPI_DOUBLE, this->hor_src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //receives ghost
                for(long j = this->j; j < this->J; j++)
                {
            ...
            if(this->hor_dest >= 0) // right
            {
                ...
                MPI_Recv(&rightGhost.front(), rightGhost.size(), MPI_DOUBLE, this->hor_dest, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //receives ghost
                for(long j = this->j; j < this->J; j++)
                {
