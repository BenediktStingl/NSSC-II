...
def createFile1(Epots, Ekins, time_steps, M, L):
    ...
    for k in range(time_steps):
        timestep, E_pot, E_kin = k, Epots[k], Ekins[k]
...
def slicer(coordinates, i): return coordinates[i*3*M : i*3*M+3], coordinates[i*3*M+3:(i+1)*3*M]

#@jax.jit
@partial(jit, static_argnums=(3,4))
def get_av_vol_density(r_list, coordinates, M, time_steps, twentyfive_percent):
    vol_densitys = []
    ...
    for r in r_list:
        ...
        for i in range(twentyfive_percent, time_steps):
            # choose first particle as center:
            center_coordinates, other_coordinates = slicer(coordinates, i)
        ...
        average = inside_particles/(time_steps-twentyfive_percent)
...
if __name__ == "__main__":
    ...
    M, C, L, coordinates, velocities, time_steps_liste = readFile(path)
    time_steps = len(time_steps_liste)
    Epots = Get_Epots(coordinates, L)
    Ekins = Get_Ekins(velocities, M, time_steps)
    print("Epots: ", len(Epots))
    print("Ekins: ", len(Ekins))
    #print(len(time_steps))
    createFile1(Epots, Ekins, time_steps, M, L)
    rs = get_r(5, L)
    ava_volumetric_density = get_av_vol_density(rs, coordinates, M, time_steps, round(time_steps/4))
    #print(len(rs))
    ...
    time_step_array = np.linspace(0, time_steps, time_steps)
